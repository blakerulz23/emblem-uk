 ...
 * no reason to share one, not a second implementation. The raw token is
 * generated the same way (randomBytes(32).toString('base64url'), see
 * src/middleware.ts's ensureBuilderCsrfCookie), set non-httpOnly and
 * sameSite:strict on first legitimate visit to /builder or /test-print.
 */
