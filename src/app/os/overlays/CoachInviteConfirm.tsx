...
export type CoachInviteLookupResult = {
  code: string;
  player: {
    firstName: string;
    lastInitial: string;
...
export type CoachInviteConfirmFields = { displayName: string };

/**
 * Mirrors TeamInviteConfirm.tsx's shape for a direct coach connection
 * instead of a team — shows only the safe context GET /api/os/coach-
 * invites/redeem returns (first name + last initial, team/club if the
 * player happens to have one), never a raw player id or the invite code
 * itself. No "already claimed" branch — a player can have more than one
 * coach (team and/or direct), so redemption isn't gated on exclusivity
 * the way a physical card claim is.
 */
export default function CoachInviteConfirm({
  result,
  onConfirm,
  onBack,
}: {
  result: CoachInviteLookupResult;
  onConfirm: (fields: CoachInviteConfirmFields) => void;
  onBack: () => void;
}) {
  const [displayName, setDisplayName] = useState('');
  const { firstName, lastInitial, team, club } = result.player;
  ...
  return (
    ...
    >
      ...
      <div style={{ fontFamily: 'Roboto', fontWeight: 900, fontSize: 26, color: '#F4F1EC', marginBottom: 8 }}>
        Connect as {firstName} {lastInitial}.&apos;s coach?
      </div>
      {(team || club) && (
        <div style={{ fontSize: 14, color: '#B8AE9F', marginBottom: 24 }}>
          {[club?.name, team?.name].filter(Boolean).join(' ')}
        </div>
      )}

      <div style={{ width: '100%', maxWidth: 300, display: 'flex', flexDirection: 'column', gap: 12, marginTop: (team || club) ? 0 : 24 }}>
        <input
          type="text"
          required
          value={displayName}
