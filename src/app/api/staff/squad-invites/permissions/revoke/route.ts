...
export async function POST(request: NextRequest) {
  ...
  const { data, error } = await service.rpc('revoke_squad_invite_staff_permission', {
    p_staff_profile_id: staffProfileId, p_permission: permission, p_revoked_by_staff_profile_id: staff.userId,
  });
  if (error) return NextResponse.json({ error: error.message || 'Could not revoke this permission' }, { status: 409 });
