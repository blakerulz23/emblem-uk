...
export async function POST(request: NextRequest) {
  logBuilderAuthorityStage('request-code:received');
  ...
  if (!(await consumeAnonymousRequestRateLimit(request.headers, 'builder-authority-otp-request', email))) {
    logBuilderAuthorityStage('request-code:rate-limited');
  ...
  if (error) {
    ...
    logBuilderAuthorityStage('request-code:otp-error');
  ...
  logBuilderAuthorityStage('request-code:success');
