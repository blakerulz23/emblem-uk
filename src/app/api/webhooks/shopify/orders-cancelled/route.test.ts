...
const WEBHOOK_ID = 'whid_cancel_1';
...
function post(payload: unknown, opts: { skipSignature?: boolean; skipWebhookId?: boolean } = {}) {
  ...
  return POST(new NextRequest('http://localhost/api/webhooks/shopify/orders-cancelled', { method: 'POST', body, headers }));
}

const PAYLOAD = { id: 555, note_attributes: [{ name: 'Order Ref', value: 'EMB-XYZ789' }] };
...
describe('POST /api/webhooks/shopify/orders-cancelled', () => {
  ...
  it('200s harmlessly with no order_ref', async () => {
    const res = await post({ id: 1 });
    expect(res.status).toBe(200);
    expect(mockRpc).not.toHaveBeenCalled();
  });

  it('200s harmlessly with no matching order', async () => {
  ...
  it('applies via apply_gate3_payment_event with the webhook id as the idempotency key and to_status cancelled', async () => {
    const res = await post(PAYLOAD);
    expect(res.status).toBe(200);
