...
const states = [
  ...
  ['board', 'Team gallery (parent landing)'], ['photo', 'Drop the photo'],
...
function CardGallery({ highlightNew = false }: { highlightNew?: boolean }) {
  ...
  const yours = highlightNew ? ROSTER.find((r) => r.id === 'p9') : null;
  return <div className="si-gallery-wrap"><div className="si-gallery" role="list" aria-label="Cards the team has already made">
    {yours && <div role="listitem" className="si-gallery-item yours"><ProductCard mini player={{ name: yours.name, number: yours.number }}/><span className="si-gallery-tag">Yours ✓</span></div>}
    {doneCards.map((r) => <div key={r.id} role="listitem" className="si-gallery-item"><ProductCard mini player={{ name: r.name, number: r.number }}/></div>)}
    <div role="listitem" className="si-gallery-item ghost"><ProductCard mini generic/><span className="si-gallery-tag">This could be yours</span></div>
  </div><p className="si-gallery-count">{doneCards.length + (yours ? 1 : 0)} cards made so far · scroll to see the team</p></div>;
}
