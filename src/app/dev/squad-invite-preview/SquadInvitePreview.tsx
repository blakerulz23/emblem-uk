...
function ProductCard({ coach = false, generic = false }: { coach?: boolean; generic?: boolean }) {
  const initials = coach ? 'CJ' : generic ? '★' : 'MR';
  const name = coach ? 'COACH JORDAN' : generic ? 'YOUR PLAYER' : 'MAYA R.';
  const detail = coach ? 'HEAD COACH' : generic ? '00 · YOUR POSITION' : '08 · MIDFIELDER';
  ...
  return <div className={`si-product-card${generic ? ' generic' : ''}`} aria-label={label}><div className="si-card-shine"/><ClubBadge className="si-card-badge"/><span className="si-card-kicker">ASHTON JUNIORS</span><div className="si-card-avatar" aria-hidden="true">{initials}</div><strong>{name}</strong><small>{detail}</small><b>EMBLEM</b></div>;
}
