...
function BrandHeader() {
  return <header className="si-brand"><button type="button" className="si-icon-button" aria-label="Go back">←</button><div className="si-brand-lockup"><Image src="/embm-nav.png" width={108} height={32} style={{ height: 'auto' }} alt="Emblem"/><span>Squad Invite</span></div><button type="button" className="si-help-button">Help</button></header>;
}
...
export default function SquadInvitePreview() {
  ...
  const screen = useMemo(() => {
    switch (state) {
      case 'create': return <><Hero eyebrow="Organiser setup" title="Create a Squad Invite" copy="One team link. Each parent builds and pays individually."/><FormRows rows={['Ashton Juniors U10', 'Under 10', 'Coach Jordan', dateText, 'Expected: 15 parents']}/><Check text="I am an adult authorised to create and share this Squad Invite."/><button className="si-primary" onClick={() => setState('approval')}>Send for staff approval <span>→</span></button></>;
