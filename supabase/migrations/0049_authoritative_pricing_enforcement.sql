-- Stage 6 pricing-integrity amendment.
--
-- Migration 0048 is already installed and immutable. Its RPC checked pricing
-- coherence but accepted coherent non-canonical prices and any positive pricing
-- version. Replace the same public signature with a complete hardened body.
--
-- PRICING CONTRACT v1 (kept in sync by static tests with
-- src/lib/pricing-engine.ts): GBP; single = 1 paid player at 2499 pence per
-- print; multi = 2-9 at 2199; squad = 10+ at 1899; subtotal is canonical
-- unit price times total print quantity; squad alone receives exactly one
-- zero-price coach-card line. Version 1 is intentionally pinned. Any future
-- price or version change requires coordinated application and database
-- migrations in the same rollout.
--
-- This migration changes no table, grant, RLS policy, or default privilege.
-- The function remains atomic, SECURITY DEFINER, owned by postgres, fixed to
-- an empty search_path, and executable only by service_role.
...
declare
  ...
  -- PRICING CONTRACT v1 (must match src/lib/pricing-engine.ts):
  -- GBP; single 1 @ 2499; multi 2-9 @ 2199; squad 10+ @ 1899.
  v_expected_tier text;
  v_expected_unit_price_pence integer;
  v_expected_subtotal_pence bigint;
begin
  ------------------------------------------------------------------------
  ...
  ------------------------------------------------------------------------
  -- Authoritative pricing contract, pinned deliberately to version 1.
  -- These constants duplicate src/lib/pricing-engine.ts by necessity at
  -- the trust boundary; the migration contract test compares both sources.
  -- Any future price or version change requires coordinated application AND
  -- database migrations. Never relax this to accept an unknown version.
  ------------------------------------------------------------------------
  if p_pricing_version is distinct from 1 then
    raise exception 'unsupported pricing version';
  end if;
  if p_currency is distinct from 'GBP' then
    raise exception 'unsupported currency';
  end if;
  if p_paid_player_count is null or p_paid_player_count < 1 then
    raise exception 'paid player count must be positive';
  end if;
  if p_total_print_quantity is null or p_total_print_quantity < 1 then
    raise exception 'total print quantity must be positive';
  end if;

  if p_paid_player_count = 1 then
    v_expected_tier := 'single';
    v_expected_unit_price_pence := 2499;
  elsif p_paid_player_count between 2 and 9 then
    v_expected_tier := 'multi';
    v_expected_unit_price_pence := 2199;
  else
    v_expected_tier := 'squad';
    v_expected_unit_price_pence := 1899;
  end if;

  if p_pricing_tier is distinct from v_expected_tier then
    raise exception 'pricing tier does not match paid player count';
  end if;
  if p_unit_price_pence is distinct from v_expected_unit_price_pence then
    raise exception 'unit price does not match authoritative pricing version 1';
  end if;

  -- Both operands originate as int4, so widening before multiplication is
  -- overflow-safe in int8. The result must still fit the approved persisted
  -- int4 money columns before it is cast back for insertion.
  v_expected_subtotal_pence :=
    p_total_print_quantity::bigint * v_expected_unit_price_pence::bigint;
  if v_expected_subtotal_pence < 0 or v_expected_subtotal_pence > 2147483647 then
    raise exception 'authoritative subtotal is outside the approved integer range';
  end if;
  if p_subtotal_pence::bigint is distinct from v_expected_subtotal_pence then
    raise exception 'subtotal does not match authoritative pricing version 1';
  end if;
  ...
  if p_coach_card is not null then
    if v_expected_tier is distinct from 'squad' then
  ...
  elsif v_expected_tier = 'squad' then
  ...
  begin
    insert into public.orders (
      ...
      'GBP', v_expected_tier, p_paid_player_count, p_total_print_quantity,
      v_expected_unit_price_pence, v_expected_subtotal_pence::integer, 1, now()
    )
  ...
  values (v_order_id, 'player_card', v_player_card_description, p_total_print_quantity,
    v_expected_unit_price_pence, v_expected_subtotal_pence::integer, 'GBP');
...
-- CREATE OR REPLACE preserves the existing owner, and this explicit owner
-- assertion documents and locks the SECURITY DEFINER trust boundary.
